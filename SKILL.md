...
description: Deploy a clean, read-only browser interface for a local Markdown directory with nested navigation, automatic file refresh, article outline, local image support, responsive layout, and light/dark themes. Use when a user asks to organize, browse, present, or continuously view many .md files in a webpage, or asks to deploy a Markdown knowledge library for Codex or another agent's output.
---
...
For a noninteractive agent check, run the server without `--open`, then verify both endpoints:
...
- Poll the tree for changes and refresh the open document without losing its reading position.
